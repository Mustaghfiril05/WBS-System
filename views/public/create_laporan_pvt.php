        ...
        <div class="row">
          <div class="col-sm-6 col-md-6"> 
              ...
              <div class="form-group">
                <strong for="lapor_sebelum">Apakah Anda Ingin Mencantumkan data pribadi?</strong>
                <select class="form-control" id="data_show" name="data_show" >
                  <option value="N">TIDAK</option>
                  <option value="Y">YA</option>
                </select>
              </div>

          <div class="form-group" id="div_data_show">
               <div class="form-group" id="div_data_diri">
                <strong >Nama</strong>
                <input type="text" class="form-control" id="nama_pelapor" name="nama_pelapor" placeholder=" " > 
               <!--  <strong >Nomor Telepon</strong>
                ...
                <strong >Alamat</strong>
                <textarea class="form-control" id="alamat_pelapor" name="alamat_pelapor" placeholder=""></textarea>
                <strong >Jabatan </strong><em>(Khusus Pegawai PT. Terminal Teluk Lamong)</em>
                <input type="text" class="form-control" id="sub_directorate" name="sub_directorate" placeholder=" "> 
                <strong >Alamat Email <a  style="text-align: center;color:#c0392b">*</a></strong>
                <input type="email" class="form-control" id="email_pelapor" name="email_pelapor" placeholder="Masukkan email"> 
              </div>
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="masalah" name="masalah" title="--Pilih Pelanggaran--">   
                  <option value="">--Pilih Pelanggaran--</option>
                  <?php 
                      foreach($optionmasalah->result() as $row)
                      { 
                        echo '<option enable-nominal="'.$row->is_kerugian.'" value="'.$row->id_masalah.'">'.$row->jenis_masalah.'</option>';
                      }
                  ?>
                </select>
              </div>
              <div class="form-group" id="div_jumlah_kerugian" style="display:none">
                <strong >Nominal Kerugian (Rp)</strong>
                <input type="number" class="form-control" name="jumlah_kerugian"  placeholder="Jumlah Kerugian Dalam Angka" > 
              </div>
                <div class="form-group">
...
<script type="text/javascript">
  ...
  $("#data_show").bind("change", function() {
    if ($(this).val() == "Y") {
      $("#div_data_show").show();
      document.getElementById("email_pelapor").required = true;
    }else{ $("#div_data_show").hide();
      document.getElementById("email_pelapor").required = false;
    }
  });

  $("#masalah").change(function(){
    if($(this).val()!=""){
      var enable=$('option:selected', this).attr('enable-nominal');
      if(enable==1){
        $("#div_jumlah_kerugian").slideDown('fast');
      }else{
        $("#div_jumlah_kerugian").slideUp('fast');
      }
    }else{
      $("#div_jumlah_kerugian").slideUp('fast');
    }
